...
<div class="container <?= require_once('background.php'); ?>">
    ...
    <form id="idForm" action="./admin/withDraw.php" method="POST">
        <div class="form-group">
            <label for="input_credit_id">Credit ID</label>
            <input type="text" class="form-control" id="input_credit_id" placeholder="123xxx" name="credit_id" required />
        </div>
        ...
        <div class="row">
            ...
            <div class="col-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="input_cvv_code" placeholder="1xx" name="cvv_code" required />
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col">
                <div class="form-group">
                    <label for="input_withdraw_money">Amount</label>
                    <input type="number" class="form-control" id="input_withdraw_money" placeholder="1000" min="0" name="withdraw_money" required value="50000" />
                </div>
            </div>
            <div class="col-4">
                <div class="form-group">
...
<script>
    $("input[name='withdraw_money']").keyup(function (e) {
        $("input[name='fee']").val(Math.round(parseInt($("input[name='withdraw_money']").val()) * 0.05));
    });
    $("input[name='withdraw_money']").change(function (e) {
        $("input[name='fee']").val(Math.round(parseInt($("input[name='withdraw_money']").val()) * 0.05));
    });
    ...
    $("#idForm").submit(function (e) {
        ...
        } else if ($("input[name='withdraw_money']").val() % 50 != 0 || $("input[name='withdraw_money']").val() == 0) {
            $(".toast-header > strong").html("Amount incorrect");
            $(".toast-body").html(`Requires greater than zero and is a multiple of 50,000<br> Please enter again`);
            $(".toast").toast("show");
            $("input[name='withdraw_money']").focus();
        } else {
            let form = $(this);
